...
def save_detections(image, detections, class_names):
    ...
    for class_id, detections_list in highest_conf_per_class.items():
        key = str(class_id)  # use string keys for JSON
        if key not in image_log:
            image_log[key] = []
        image_log[key].extend(detections_list)
    ...
    updated_names = list(set(existing_names + [
        class_names[int(cid)] for cid in detected_class_ids if int(cid) < len(class_names)
    ]))
...
async def summarize_detected_objects(video_path: str, situation: str) -> str:
    ...
    with open("class_names.txt", "r") as f:
        event = f.read()
    input = object_list + "\n" + event
    response = await journal_agent.run(user_prompt=input)
    return response
